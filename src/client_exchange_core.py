...
def _android_id() -> str:
    """شناسه پایدار دستگاه اندروید (ANDROID_ID)، در صورت اجرا روی اندروید."""
    try:
        from jnius import autoclass  # type: ignore
        PythonActivity = autoclass("org.kivy.android.PythonActivity")
        context = PythonActivity.mActivity.getApplicationContext()
        Settings_Secure = autoclass("android.provider.Settings$Secure")
        value = Settings_Secure.getString(context.getContentResolver(), "android_id")
        return str(value or "")
    except Exception:
        return ""
...
def open_client_package(path: str, admin_keys: AdminKeyStore,
                        client_sign_public: str) -> Dict[str, Any]:
    try:
        envelope = json.loads(Path(path).read_text(encoding="utf-8"))
    except Exception as exc:
        raise ExchangeError("فایل خروجی کلاینت قابل خواندن نیست.") from exc
    if envelope.get("format") != PACKAGE_FORMAT or int(envelope.get("version") or 0) != FORMAT_VERSION:
    ...
    try:
        ...
        raise DecryptionError("رمزگشایی فایل کلاینت ناموفق بود؛ فایل ممکن است آسیب‌دیده یا دست‌کاری شده باشد.") from exc
